...
for fn in os.listdir('data'):
    if fn.endswith('.csv'):
        ...
        if data[0][2:5] == ['Status', 'Secondary_Status', 'Arrival_Date']:
            ...
            te_data = data
            te_data_type = 'SPH'
        elif data[0][:2] == ['Date', 'Folio Id']:
            te_data = data
            te_data_type = 'HelloGM'
...
def process_te_sph(te_data, cleaned_data):
...
def process_te_hgm(te_data, cleaned_data):
...
def process_guest(guest_data, cleaned_data):
...
if te_data and guest_data:
    ...
    if te_data_type == 'SPH':
        cleaned_data = process_te_sph(te_data, cleaned_data)
    elif te_data_type == 'HelloGM':
        cleaned_data = process_te_hgm(te_data, cleaned_data)

    cleaned_data = process_guest(guest_data, cleaned_data)
